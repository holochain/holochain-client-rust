...
use holochain_client::{AdminWebsocket, AppAgentWebsocket, InstallAppPayload, InstalledAppId};
...
async fn network_info() {
    ...
    let mut app_ws = AppAgentWebsocket::connect(format!("ws://localhost:{}", app_ws_port), app_id)
        .await
        .unwrap();
...
async fn call_zome() {
    ...
    let app_ws_port = 33000;
    admin_ws.attach_app_interface(app_ws_port).await.unwrap();
    let mut app_agent_ws = AppAgentWebsocket::connect(format!("ws://localhost:{}", app_ws_port), app_id)
        .await
        .unwrap();
    ...
    let network_info = app_agent_ws
        .call_zome(AppAgentNetworkInfoRequestPayload {
            dnas: vec![dna_hash],
            last_time_queried: None,
        })
        .await
